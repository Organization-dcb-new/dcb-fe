...
const ReportDownload: React.FC = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      setFilteredClient('Coda')
      setFilteredPaymentMethod({
        name: 'Telkomsel',
        value: 'telkomsel_airtime',
      })
      setData(res.data)
  ...
  const exportToPDF = () => {
    ...
    const tableWidthsTan = columns.map(() => 'auto')
    // const tableWidthsTan = ['15%', '15%', '10%', '20%', '20%', '20%']
    ...
    const tableBodyTan: TableCell[][] = [
      columns.map((column) => ({
        text: String(column.columnDef.header ?? ''),
        bold: true,
      })),
      ...
      ...table.getRowModel().rows.map((row) =>
        row.getVisibleCells().map((cell) => {
          ...
          return {
            text: typeof value === 'number' ? numFmt.format(value) : String(value ?? ''),
    ...
    const totalInWords = numberToWords.toWords(total || 0).replace(/\b\w/g, (l: any) => l.toUpperCase()) + ' Rupiah'
